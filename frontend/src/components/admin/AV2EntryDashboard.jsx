...
import { RefreshCw, Download, BarChart3, Star, Ticket, Store, Handshake, Users } from 'lucide-react';
...
const AV2EntryDashboard = () => {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [selectedRange, setSelectedRange] = useState(() => {
    const params = new URLSearchParams(window.location.search);
    return params.get('range') || '30d';
  });
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const result = await adminApi.getAV2EntryDashboard(params);
      setData(result);
      setLastUpdated(new Date());
    } catch (err) {
      console.error('Failed to load AV2 Entry dashboard data', err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const metrics = [
    {
      ...
      helper: 'VIP access revenue only',
      icon: BarChart3,
      tone: 'rev',
    },
    {
      key: 'vip',
      label: 'VIP Revenue',
      value: INR(summary.vip_revenue),
      helper: `${summary.vip_count || 0} bookings, ${summary.checked_in || 0} checked in`,
      icon: Star,
      tone: 'bk',
    },
    {
      key: 'free',
      label: 'Free Entries',
      value: (summary.free_count || 0).toLocaleString('en-IN'),
      helper: `${summary.free_tickets || 0} tickets, ${summary.checked_in || 0} checked in`,
      icon: Ticket,
      tone: 'ok',
    },
    {
      ...
      value: (summary.stall_count || 0).toLocaleString('en-IN'),
      helper: `${summary.sponsor_count || 0} sponsorships`,
      icon: Store,
      tone: 'in',
    },
  ];
  ...
  const breakdownData = {
    labels: ['VIP Registrations', 'Free Entries', 'Stall Bookings', 'Sponsorships'],
    datasets: [
      {
        data: [
          Number(summary.vip_count || 0),
          Number(summary.free_count || 0),
          Number(summary.stall_count || 0),
          Number(summary.sponsor_count || 0),
        ],
        backgroundColor: ['#1D4ED8', '#28A745', '#D35400', '#B8860B'],
        borderWidth: 0,
        hoverOffset: 6,
      },
    ],
  ...
  const exportCsv = () => {
    ...
    link.href = url;
    link.download = `av2-entry-dashboard-${Date.now()}.csv`;
  ...
  if (loading) {
    return (
      <div className="page on">
        <div className="ph">
          <div>
            <div className="ph-title">AV2 Entry Dashboard</div>
            <div className="ph-sub">Loading event analytics...</div>
          </div>
        </div>
    ...
    <div className="page on dash-page">
      ...
      <div className="dash-shell">
        <div className="dash-head">
          <div>
            <h1 className="dash-title">AV2 Entry Dashboard</h1>
            <div className="dash-sub">
              VIP access, free entries, stall bookings, and sponsorships overview.
            </div>
          </div>
        ...
        <div className="dash-grid dash-hero">
          <div className="dash-card">
            <div className="dash-card-hd">
              <div>
                <div className="dash-card-title">Daily Trend</div>
                <div className="dash-card-sub">VIP revenue and registrations by day.</div>
              </div>
          ...
          <div className="dash-card">
            <div className="dash-card-hd">
              <div>
                ...
                <div className="dash-card-sub">Checked-in vs registered visitors.</div>
              </div>
              <Users size={16} color="var(--tx3)" />
            </div>
            <div className="dash-card-bd">
              ...
              <div className="dash-split" style={{ marginTop: 14 }}>
                <div className="dash-mini">
                  <div className="dash-mini-k">Total Registered</div>
                  <div className="dash-mini-v">{(summary.total_registered || 0).toLocaleString('en-IN')}</div>
                </div>
                <div className="dash-mini">
                  <div className="dash-mini-k">Checked In</div>
                  <div className="dash-mini-v">{(summary.checked_in || 0).toLocaleString('en-IN')}</div>
                </div>
                <div className="dash-mini">
                  <div className="dash-mini-k">Attendance Rate</div>
                  <div className="dash-mini-v">{summary.attendance_rate || 0}%</div>
                </div>
                <div className="dash-mini">
          ...
          <div className="dash-card">
            <div className="dash-card-hd">
              <div>
                ...
                <div className="dash-card-sub">Latest VIP, free, and stall registrations.</div>
              </div>
            ...
            <div className="dash-card-bd">
              <div className="dash-list">
                {recentTransactions.length > 0 ? recentTransactions.map((row, idx) => (
                  <div className="dash-row" key={`${row.type}-${row.id || idx}`}>
                    <div>
...
export default AV2EntryDashboard;
